...
# ========== 所属概念词云分析（可折叠） ==========
with st.expander("📊 所属概念词云分析", expanded=False):
    # 检查是否有概念数据
    if stock_concepts:
        # 收集所有概念
        all_concepts = []
        for concept_str in stock_concepts.values():
            if pd.notna(concept_str) and concept_str not in ['未知', 'nan', 'NaN', 'None']:
                # 拆分多概念（支持分号、逗号分隔）
                concepts = str(concept_str).replace('；', ';').replace('，', ',').replace('、', ';')
                for single_concept in concepts.split(';'):
                    for sub_concept in single_concept.split(','):
                        cleaned_concept = sub_concept.strip()
                        if cleaned_concept and len(cleaned_concept) > 1:
                            all_concepts.append(cleaned_concept)
        ...
        if all_concepts:
            ...
            concept_df = pd.DataFrame({
                '概念': list(concept_counts.keys()),
                '出现次数': list(concept_counts.values())
            }).sort_values('出现次数', ascending=False)
            
            # 显示概念统计表格
            st.write(f"**概念统计（共 {len(concept_df)} 个不同概念）**")
            col1, col2 = st.columns([2, 1])
            
            with col1:
                # 显示前20个概念
                st.dataframe(
                    concept_df.head(20).reset_index(drop=True),
                    use_container_width=True
                )
            
            with col2:
                # 显示统计信息
                st.metric("总概念出现次数", len(all_concepts))
                st.metric("不同概念数量", len(concept_df))
                if len(concept_df) > 0:
                    st.metric("最热门概念", f"{concept_df.iloc[0]['概念']} ({concept_df.iloc[0]['出现次数']}次)")
            ...
            try:
                ...
                if font_path is None:
                    st.warning("未找到合适的中文字体，词云可能无法正确显示中文。尝试使用备用方案...")
                    # 尝试使用matplotlib的字体
                    try:
                        import matplotlib.font_manager as fm
                        font_path = fm.findfont(fm.FontProperties(family=['SimHei', 'Microsoft YaHei', 'DejaVu Sans']))
                    except:
                        st.error("无法找到可用的中文字体，词云将显示为方框")
                        font_path = None
                
                # 准备词云文本（直接使用概念，不进行分词）
                concept_freq_dict = dict(concept_counts)
                ...
                wordcloud = WordCloud(
                    ...
                    random_state=42
                ).generate_from_frequencies(concept_freq_dict)
                
                # 显示词云
                fig, ax = plt.subplots(figsize=(12, 6))
                ax.imshow(wordcloud, interpolation='bilinear')
                ax.axis('off')
                ax.set_title('所属概念词云分析', fontsize=16, pad=20)
                
                # 添加说明文本
                plt.figtext(0.5, 0.01, 
                           f"基于 {len(all_concepts)} 个概念出现次数生成 • 字体大小表示概念热度", 
                ...
                st.error(f"生成词云需要额外依赖包: {e}")
                st.info("请安装所需依赖: `pip install wordcloud`")
                
            except Exception as e:
                st.error(f"生成词云时出错: {e}")
                # 显示备选方案：条形图
                st.info("使用条形图显示热门概念:")
                top_concepts = concept_df.head(15)
                
                fig, ax = plt.subplots(figsize=(10, 6))
                y_pos = np.arange(len(top_concepts))
                
                ax.barh(y_pos, top_concepts['出现次数'], color='skyblue', alpha=0.7)
                ax.set_yticks(y_pos)
                ax.set_yticklabels(top_concepts['概念'], fontsize=10)
                ax.invert_yaxis()  # 从上到下显示
                ax.set_xlabel('出现次数')
                ax.set_title('Top 15 热门概念')
                
                # 在条形上添加数值
                for i, v in enumerate(top_concepts['出现次数']):
            ...
            # 概念与股票关联分析
            st.subheader("🔗 概念与股票关联分析")
            
            # 创建概念-股票映射
            concept_stocks = {}
            for code, concept_str in stock_concepts.items():
                if pd.notna(concept_str) and concept_str not in ['未知', 'nan', 'NaN', 'None']:
                    concepts = str(concept_str).replace('；', ';').replace('，', ',').replace('、', ';')
                    for single_concept in concepts.split(';'):
                        for sub_concept in single_concept.split(','):
                            cleaned_concept = sub_concept.strip()
                            if cleaned_concept and len(cleaned_concept) > 1:
                                if cleaned_concept not in concept_stocks:
                                    concept_stocks[cleaned_concept] = []
                                concept_stocks[cleaned_concept].append(code)
            
            # 显示每个概念对应的股票
            if concept_stocks:
                selected_concept = st.selectbox(
                    "选择概念查看对应股票",
                    options=sorted(concept_stocks.keys()),
                    index=0
                )
                
                if selected_concept:
                    stocks_in_concept = concept_stocks[selected_concept]
                    st.write(f"**概念 '{selected_concept}' 包含 {len(stocks_in_concept)} 只股票:**")
                    
                    # 显示股票详情
                    concept_stock_details = []
                    for code in stocks_in_concept:
                        stock_name = next((r['股票简称'] for r in all_results if r['股票代码'] == code), '未知')
                        concept_stock_details.append({
                            '股票代码': code,
                            '股票简称': stock_name,
                            '同花顺链接': generate_ths_link(code)
                        })
                    
                    if concept_stock_details:
                        concept_df_display = pd.DataFrame(concept_stock_details)
                        # 使用HTML显示带链接的表格
                        concept_df_display['股票代码'] = concept_df_display.apply(
                            lambda x: f'<a href="{x["同花顺链接"]}" target="_blank">{x["股票代码"]}</a>', 
                            axis=1
                        )
                        concept_df_display = concept_df_display[['股票代码', '股票简称']]
                        
                        st.write(concept_df_display.to_html(escape=False, index=False), unsafe_allow_html=True)
            else:
                st.info("未找到有效的概念-股票映射数据")
        
        else:
            st.info("未找到有效的概念数据进行词云分析")
    else:
        st.info("暂无概念数据可用于词云分析")
